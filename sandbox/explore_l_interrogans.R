...
roary_pagoo <- roary_2_pagoo("outputs/roary/s__Leptospira-interrogans/gene_presence_absence.csv")
pk10_pagoo <- read_long_sketch_table_as_pagoo("outputs/sourmash_sketch_tables/protein-k10_scaled100/s__Leptospira-interrogans_long.csv")


compare_summary_stats(roary_pagoo, pk10_pagoo)


# redo corr ---------------------------------------------------------------


tmp <- correlate_total_per_genome(roary = roary_pagoo$pan_matrix,
                                  mers = pk10_pagoo$pan_matrix, 
...
roary <- read_roary_presence_absence("outputs/roary/s__Leptospira-interrogans/gene_presence_absence.csv")
pk10 <- read_mers_presence_absence("outputs/sourmash_sketch_tables/protein-k10_scaled100/s__Leptospira-interrogans_wide.feather")

tmp2 <- correlate_total_per_genome(roary = roary,
                                   mers = pk10, 
                                   species = "L. interrogans", 
                                   alpha = "protein", 
                                   ksize = 10, 
...
# gca_001292615_2: Excluded from RefSeq: 
# many frameshifted proteins 
# missing tRNA genes
# gca_002269235_1: Excluded from RefSeq: 
# many frameshifted proteins 
# missing ribosomal protein genes
# gcf_008373805_1: Excluded from RefSeq: 
# many frameshifted proteins
# gca_000244435_3: Excluded from RefSeq:
# many frameshifted proteins
# gca_000216295_3


# systematically label outliers -------------------------------------------
...
total_per_genome <-total_per_genome %>%
  ...
  left_join(genbank, by = "assembly_accession")
...
# more than just the ones colored on the plot are excluded from refseq for frame shift,
# not sure why they don't appear on the spreadsheet


total_per_genome2 <- total_per_genome %>%
  filter(is.na(excluded_from_refseq))

lm_result <- lm(total_genes_per_genome ~ total_mers_per_genome, data = total_per_genome2) %>%
  glance() %>%
  mutate(species = species)
...
ggplot(total_per_genome2, 
       aes(x = total_genes_per_genome, y = total_mers_per_genome)) +
  geom_point() +
  theme_minimal() +
  theme(axis.title = element_text(size = 8),
        plot.title = element_text(size = 9),
...
pk10_pagoo$summary_stats
pk10_pagoo$gg_barplot()
pk10_pagoo$gg_curves()
pk10_pagoo$gg_binmap()
pk10_pagoo$gg_dist()
pk10_pagoo$gg_pca()
pk10_pagoo$gg_pie()
